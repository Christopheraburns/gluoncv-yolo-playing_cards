...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = VOCLike(root='/opt/ml/input/data/train', splits=((2019, 'train'),))
        val_dataset = VOCLike(root='/opt/ml/input/data/train', splits=((2018, 'val'),))
...
def model_fn(model_dir):
    ...
    """
    net = get_model("yolo3_darknet53_custom", classes = num_classes, ctx=mx.gpu(0) )
    s3.meta.client.download_file("remars2019-yolovoc-format", "round2.params", 'model.params')
...
def transform_fn(net, request_body, input_content_type, output_content_type):
    """
    ...
    s3_uri = urlparse(parsed['s3Uri'])
    s3_bucket = s3_uri.netloc
    key = s3_uri.path.lstrip('/')
    filename = os.path.basename(s3_uri.path)
    s3.meta.client.download_file(s3_uri.netloc, s3_uri.path.lstrip('/'), '/tmp/' + filename)
    x, image = gcv.data.transforms.presets.yolo.load_test('/tmp/'+filename, 608)
...
if __name__ =='__main__':
    ...
    parser.add_argument('--model-file', type=str, default='s3://bucket/file.model.param')
    ...
    if args.syncbn and len(ctx) > 1:
        ...
        async_net = get_model(net_name, pretrained_base=False, classes=num_classes)  # used by cpu worker
    else:
        net = get_model(net_name, classes=num_classes, pretrained_base=True)
        async_net = net
    if 's3' in args.resume:
        o = urlparse(args.resume)
        s3.meta.client.download_file(o.netloc, o.path.lstrip('/'), './resume.params')
        net.load_parameters('resume.params')
        async_net.load_parameters('resume.params')
    else:
    ...
    # training
    train_job(net, train_data, val_data, eval_metric, ctx, args)
    
    for file in os.listdir('.'):
        if 'custom_0000' in file:
            shutil.move(file, os.path.join(args.model_dir, 'model.params'))
